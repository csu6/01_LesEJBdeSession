...
@Stateless
@LocalBean
// Un bean de session sans état sera utiliser par des clients différents
// Si un client stocke une valeur dans ce genre de bean, un autre client
// peut la lire.
// Un bean sans état est partagé par tous les clients.
// servira des clients différents à tour de rôle.

// A la différence, un bean avec état (voir le bean Echo), n'affichera la donnée stockée 
// qu'au client qui a déjà utilisé ce bean.
// Un bean avec état a de la mémoire: le même bean sera mise à la disposition du même client.
// Chaque client a son propre bean avec état.

// La seule différence entre un bean avec état et un bean sans état se situe au niveau de
...
//    }
    ...
    {
        //créer un objet de formatage de date de type sdf(SimpledateFormat)            
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        ...
        {
            System.out.println("Une Exception a été rencontrée : " +
                    "source : Calculette\nMessage:" + 
                     ex.getMessage());
        }
